(function() {
  var Colorlexia, ColorlexiaView, CompositeDisposable;
  ...
  function updateColors(loc){
    loc.find(enabledSourceObjects).each(function(i, el) {
      ...
      var semanticClass = "identifier colorlexia-" + colorIndex(text);
      $(el).addClass(semanticClass);
    });
  }

  function updateConfigRuntime(_this){
    var config = atom.config.get('colorlexia');

    var enableGrammarsArray = [];
    _.each(Colorlexia.config.affectedGrammars.properties, function(v, i){
    ...
    _.each(Colorlexia.config.affectedSourceObjects.properties, function(v, i){
      var status = v.default;
  ...
  module.exports = Colorlexia = {
    ...
    activate: function(state) {
      ...
      return atom.workspace.observeTextEditors(function(editor) {
        ...
        atom.config.observe('colorlexia', function(){
        ...
        editor.onDidChangeScrollTop(function(){
          colorBounce();
          /*return updateColors(shadow.find('.line'));*/
        });
        editor.onDidStopChanging(function(){
          colorBounce();
          /*return updateColors(shadow.find('.cursor-line'));*/
        });
        editor.onDidChangeCursorPosition(function(positions){
          colorBounce();
          /*return updateColors(shadow.find('[data-screen-row='+positions.oldBufferPosition.row+']'));*/
        });
        return editor.onDidChange(function() {
          colorBounce();
          /*return updateColors(shadow);*/
        });
      });
    },
    deactivate: function() {
      ...
      return this.colorlexiaView.destroy();
    },
    serialize: function() {
      return {
        colorlexiaViewState: this.colorlexiaView.serialize()
      };
    }
  };
