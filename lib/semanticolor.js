(function() {
	...
	var ignoredFileTypes = ['md', 'sh', 'cmd', 'bat'];
	var forceIncludeFileTypes = ['php', 'html', 'xml', 'diff'];

	var grammars = {};
	var options = ['contrast', 'mute', 'theme', 'colorize', 'defer'];
	var config = {
		colorOptions: {
	...
	module.exports = Semanticolor = {
		...
		activate: function(state) {
			...
			function tryAddGrammarToConfig(grammar) {
				...
				var supported = isSupportedGrammar(grammar);
				var inGrammarList = grammarList.includes(grammarName);
				...
				if (inGrammarList && supported) {
					...
					console.log('removed ' + grammarName, grammar);
			...
			this.disposables.add(atom.config.onDidChange('semanticolor', function(change) {
				...
				_.debounce(function() {
					...
					var reload = !Sugar.Object.isEqual(change.newValue.colorOptions.hues, change.oldValue.colorOptions.hues);
					for (var prop in change.newValue) {
