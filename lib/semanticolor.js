...
let ignoredFileTypes = ['md', 'cmd', 'bat', 'diff', 'Dockerfile', 'json'];
let forceIncludeFileTypes = ['sh', 'php', 'html', 'xml', 'vue', 'mjml'];
...
function enable(editor) {
	...
	if (grammars[paramName]) {
		debug('using ' + grammars[paramName].name);
...
let Semanticolor = {
	...
	activate: function () {
		...
		function createGrammar(grammar) {
			...
			if (options) {
				let newGrammar = SemanticolorGrammarFactory.create(grammar, defaults, options, cfg.colorOptions.hues);
				if (!grammars[paramName] || newGrammar.scopeMap) {
					grammars[paramName] = newGrammar;
				}
			}
			// Activate grammar on existing text editors with matching grammar
			atom.workspace.getTextEditors()
				.filter(editor => editor.getGrammar().scopeName === grammar.scopeName)
				.map(enable);
		}
		...
		function isSupportedGrammar(grammar) {
			...
			if (grammar.packageName !== SemanticolorGrammarFactory.packageName) {
				if (utils.isTreeSitter(grammar)) {
					debug('grammar (tree-sitter)', grammar);
				}
