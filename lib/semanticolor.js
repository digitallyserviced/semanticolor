...
let ignoredFileTypes = ['md', 'cmd', 'bat', 'diff', 'Dockerfile'];
let forceIncludeFileTypes = ['sh', 'php', 'html', 'xml', 'xsd', 'vue', 'mjml'];
...
function enable(editor) {
	...
	if (paramName && grammars[paramName] && grammar !== grammars[paramName]
		&& !Semanticolor.deactivated && cfg && cfg[paramName] && cfg[paramName].enabled) {
		debug('using', grammars[paramName].description, '- ' + editor.getTitle());
	...
	} else if (grammar.packageName === SemanticolorGrammarFactory.packageName
		&& (Semanticolor.deactivated || cfg && cfg[paramName] && !cfg[paramName].enabled)) {
		debug('disabled');
	}
}
	...
	activate: function () {
		...
		function createGrammar(grammar) {
			...
			if (options && (!grammars[paramName]
					|| (utils.isTreeSitter(grammar) && !utils.isTreeSitter(grammars[paramName])))) {
		...
		function isSupportedGrammar(grammar) {
			...
			if (grammar.packageName !== SemanticolorGrammarFactory.packageName) {
				if (grammar.name && grammar.scopeName.includes('source') && (!grammar.fileTypes || !grammar.fileTypes.length ||
