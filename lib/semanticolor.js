(function() {
	...
	var ignoredFileTypes = ['md', 'diff', 'json', 'cson'];
		...
		activate: function(state) {
			...
			function tryAddGrammarToConfig(grammar) {
				...
				if (grammar.packageName !== SemanticolorGrammarFactory.packageName &&
					grammar.scopeName.includes('source') && !/\..+\./.test(grammar.scopeName) &&
			...
			this.disposables.add(atom.config.onDidChange('semanticolor', function(change) {
				...
				_.debounce(function() {
					...
					if (!Sugar.Object.isEqual(change.newValue.editorOptions, change.oldValue.editorOptions)) {
						Semanticolor.confirmReload();
					}
				}, 1000)();
			}));
