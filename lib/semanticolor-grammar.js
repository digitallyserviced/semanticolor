...
function semanticolorGrammarFactory(grammar, defaults, options, colorDiversity) {
	...
	if (semanticolorGrammar.scopeMap) {
		semanticolorGrammar.name = 'semanticolor (tree-sitter) - ' + grammar.name;
		semanticolorGrammar.idForScope = (function (scopeName, text) {
			let id = this.__proto__.idForScope(scopeName);
			if (text.length > 256) {
				return id;
			}
			let { actions, includeAllChars } = determineActions(this.defaults, this.options, [scopeName]);
			let everythingElse = this.options.everythingElse === 'default' ? this.defaults.everythingElse : this.options.everythingElse;
			let newScope = getNewScope(text, actions, this.colorDiversity, includeAllChars, everythingElse);
			if (newScope && newScope !== scopeName) {
				return this.__proto__.idForScope(newScope);
			}
			return id;
