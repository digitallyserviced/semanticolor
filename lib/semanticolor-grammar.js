...
function semanticolorGrammarFactory(grammar, options, colorDiversity) {
	...
	semanticolorGrammar.options = selector;
	...
	semanticolorGrammar.tokenizeLine = function(line, ruleStack, firstLine) {
		...
		if (tokenTagIndices.length) {
			for (let i = tokens.length - 1; i >= 0; i--) {
				let actionsResult = determineActions(this.options, tokens[i].scopes);
				...
				for (let j = 0; j < subtokens.length; j++) {
					let newScope = getNewScope(subtokens[j], actions, this.colorDiversity, includeAllChars, this.options.everythingElse);
					if (newScope) {
		...
		let result = Sugar.Object.merge(tokenizeResult, { tags: tags, openScopeTokens: openScopeTokens });
		return result;
	};
